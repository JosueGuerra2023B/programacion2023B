...
void escribirArchivo1(Proforma& proforma) {
    ...
    if (archivo.is_open()) {
        ...
        for (int i = 0; i < 1; i++) { // Solo para el primer elemento, adaptar según necesidad
...
Proforma convertirStringAProforma(const std::string& fecha) {
    Proforma proforma;
...
int diferenciaDias(const std::string& fechaInicio, const std::string& fechaFinal) {
...
void registrosViaje1(){
    ...
    switch (opcion) {
        case 1:
    		...
    		switch (opcion) {
        		case 1:
            		nueva_proforma.transporte = "Autobús";
			...
			break;
        case 2:
            nueva_proforma.destino = "Montañita";
            ...
            break;
		case 3:				
            nueva_proforma.destino= "Galápagos";
            nueva_proforma.cantidadDestino=370;
            cout<<endl;
           	cout << "      Transporte del Viaje    " << endl;
			cout << "1. Avión + Taxi $265 " << endl;
    		cout << "Seleccione el destino del viaje: ";
    		cin >> opcion;
   			cin.ignore(); // Ignorar el salto de línea pendiente en el buffer
    		switch (opcion) {
    			case 1:
        			nueva_proforma.transporte= "Avión + Taxi";
    ...
    // Determinar el porcentaje de aumento según la duración del viaje
    double porcentajeAumento = 0;
	if (diasViaje > 32) {
    	porcentajeAumento = 0.12; // 12% de aumento para más de un mes 
	} else if (diasViaje >= 16 && diasViaje <= 30) {
    	porcentajeAumento = 0.10; // 10% de aumento para más de 15 días y menos de un mes 
	} else if (diasViaje <= 15) {
    	porcentajeAumento = 0.05; // 5% de aumento para 15 días o menos
	}
    ...
    switch (opcion) {
        ...
        case 2:
            ...
            break;
        case 3:
            nueva_proforma.alojamiento = "Cabaña";
